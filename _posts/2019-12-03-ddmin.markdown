...
[Pyodide](https://github.com/iodide-project/pyodide/) takes a little time
to initialize. Please wait until the *Run all* button has a red border.
...
def ddmin(cur_str, causal_fn):
    ...
    while part_len and cur_str:
        start, cur_str = remove_check_each_fragment(cur_str, start, part_len,
                                                    causal_fn)
        if start == -1:
...
def perses_delta_debug(grammar, orig_tree, predicate):
    ...
    while p_q:
        ...
        # There is a choice here. The issue is that not all grammars
