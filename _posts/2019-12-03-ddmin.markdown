...
That was ofcourse illuminating. However, is that the only way to implement this?
*delta-debug* at its heart, is a divide and conquer algorithm. Can we implement it
recursively?

#### Recursive Delta Debug `ddrmin()`
...
can not be partitioned to strictly smaller parts. So, we handle it separately.
...
def ddrmin(cur_str, causal_fn, pre, post):
    ...
    return ddrmin(string1, causal_fn, pre, string2 + post) +  ddrmin(string2, causal_fn, pre+string1, post)
