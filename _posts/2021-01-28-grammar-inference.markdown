...
* Precision (P): The percentage of inputs that were generated by the synthesized
  grammar that were accepted by the blackbox. That is, the synthesized grammar acts as a producer.
...
generate strings from the grammar. Precision and recall are then approximated by
Monte Carlo sampling. This is why random sampling is not merely an implementation
