...
So, what is next? We need to first find the most useful lexical unit empirically that can be used to determine the lexical neighborhood for a program. Our initial empirical analysis suggests that it may be expressions, statements, or functions. Next we need to identify mutation operators that can transform these lexical units such as functions correctly to emulate faults. A simple stubbing operator can emulate feature additions, and domain specific operators such as removing or rearranging synchronization primitives, or resource cleanup can emulate faults in the particular domain. While this introduces a limited context sensitivity to mutant generation, we gain the ability to incorporate more complex faults as more patterns of faults (such as the heart bleed bug) become known.
