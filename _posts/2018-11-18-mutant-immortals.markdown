...
The main assumption we made here is that all mutants are equally hard to catch. One way to overcome that is by using more complex estimators. The nice thing about mutation analysis is that one can quite accurately estimate the probability distribution for difficulty of killing by looking at the kill matrix from both methods. Further, one can refine the population estimate further by using better or more varied fuzzers. See Accettura et al.[5] for details on capture-recapture models.

However, there is a better way in mutation analysis! We have a way to quantify the difficulty in capture of mutants. We can simply consider each mutant to be representative of as many unit mutants as it was killed by the test suite. That is, if m1 was killed by 4 test cases, consider m1 equivalent to 4 unit mutants. Now, when we compute the total number of killable mutants, it will be computed in terms of unit mutants. Remove all the killed unit mutants from that number, and one is left with the unit mutants that are actually killable. Given that these mutants have more capture difficulty than what we consider a _unit_ mutant, this is likely to be an over estimate of the killable mutants, and an under estimate of the number of immortals.
