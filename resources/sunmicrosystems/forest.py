...
def diff(ui, top, *pats, **opts):
    """show diff of changed files in the working forest

    Show diff of files in this forest's repositories.

    Look at the help text for the diff command for more information.
    """
    forest = Forest(top=top, walkhg=walkhgenabled(ui, opts['walkhg']))
    die_on_numeric_revs(opts['rev'])
    # Figure out which paths are relative to which roots
    ...
    def function(tree, path, opts):
        ...
        if files:
            ...
            localui = munge_ui(prefix, ui)
        try:
            commands.diff(localui, tree.repo, *pats, **opts)
        except RepoError, err:
            ui.warn(_("skipped: %s\n") % err)
...
def fimport(ui, top, *pats, **opts):
    """import a previously generated fdiff

    Look at the help text for the import command for more information.
    """
    forest = Forest(top=top, walkhg=walkhgenabled(ui, opts['walkhg']))
    if len(pats) != 1:
        raise util.Abort(_("invalid arguments for fimport."))
    patchfile = pats[0]

    try:
        lines = [l for l in open(patchfile)]
    except:
        raise util.Abort(_("invalid argument for fimport."))
    ...
    for tree in forest.trees:
        try:
            ...
            toggle = False
            for l in lines:
                if l == path: toggle = True
                elif repo_re.match(l): toggle = False
                if toggle:
                    patchlines.append(l)
            if len(patchlines) > 2:
                fd = tempfile.NamedTemporaryFile(dir=".hg")
                print 'importing...'
                for l in patchlines: fd.write(l)
                fd.flush()
                opts['no_commit'] = True
                commands.import_(ui, tree.repo, (fd.name), **opts)
                fd.close()
...
def commit(ui, top, *pats, **opts):
    """commit all the outstanding changes in all trees with same message.
    """
    ...
    hgmessage = []
    for tree in forest.trees:
        (mod, add, rem, deleted, unknown, ign,clean) = tree.repo.status()
        hgmessage.append("HG: [repo %s]" % relpath(top.root, tree.root))
        for m in mod: hgmessage.append("HG:     M %s" % m)
        for m in add: hgmessage.append("HG:     A %s" % m)
        for m in rem: hgmessage.append("HG:     R %s" % m)

    #hgmessage = [tree.repo.status()[0] for tree in forest.trees]
    ...
    for tree in forest.trees:
        try:
            ui.write("[%s]\n" % relpath(top.root, tree.root))
            tree.die_on_mq(top.root)
            commands.commit(ui, tree.repo, *pats, **opts)
        except RepoError, err:
            ui.warn(_("skipped: %s\n") % err)
...
def uisetup(ui):
    ...
    cmdtable = {
        ...
        "^fdiff|fd" :
            (diff,
             [walkhgopts] + cmd_options(ui, 'diff'),
             _('hg fdiff [OPTION]... [FILE]...')),
        "^fimport|fi" :
            (fimport,
             [walkhgopts] + cmd_options(ui, 'import'),
             _('hg fimport [OPTION]... [FILE]...')),
        "ftrees" :
            (trees,
             [('c', 'convert', False,
        ...
        "fcommit" :
            (commit,
             [walkhgopts] + cmd_options(ui,'commit'),
             _('hg fcommit [OPTIONS]')),
