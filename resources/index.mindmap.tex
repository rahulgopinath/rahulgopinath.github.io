...
\begin{tikzpicture}[ every annotation/.style = {draw, fill = white, font = \Large}]
  \path[mindmap,concept color=,
    every node/.style={concept,fill=white},
    root/.style    = {concept color=black!80,font=\large\bfseries,text width=18em},
    level 1 concept/.append style={color=black!80,font=\Large\bfseries,sibling angle=90,text width=10.7em,level distance=20em},
    level 2 concept/.append style={color=black!80,font=\bfseries,sibling angle=50,level distance=18em, text width=10em},
    level 3 concept/.append style={color=black!80,font=\bfseries,sibling angle=50,level distance=16em, text width=10em},
  ]
  node[root] {Software Testing} [clockwise from=0]
    child[concept] {
      node[concept] {Coverage} [clockwise from=30]
    	child[concept] { node (predictionN) {Prediction} }
    	child[concept] { node (testednessN) {Testedness} }
    }
    child[concept] {
      node[concept] {Mutation Analysis} [clockwise from=0]
    	child[concept] {
          node {Theory} [clockwise from=0]
        	...
        	child { node (competentN) {Competent Programmer}  }
      }
      child[concept] {
    ...
    child[concept] {
      node[concept] {Test suites} [clockwise from=210]
    	child[concept] { node (swarmN) {Swarm Testing} }
    	child[concept] { node (reductionN) {Reduction} }
    };
    \info[10]{swarmN.north east}{above,anchor=west,xshift=-15em}{%
      \item 2016 ISSTA
    }
    ...
    \info[10]{syntacticN.east}{anchor=west,xshift = 0.5em}{%
      \item 2014 ISSRE
    }
    \info[8]{smarterN.east}{anchor=north,xshift = -14em,yshift=1em}{%
      \item 2016 ICSE
    }
    \info[10]{fasterN.east}{anchor=south,xshift = -12em,yshift=1em}{%
      \item 2016 ICSE
    }
    \info[8]{fewerN.east}{anchor=south,xshift = -14em,yshift=-2em}{%
      \item 2016 ICSE
    }
\end{tikzpicture}
\end{document}
